...
const EmpEdit = () => {
    ...
    useEffect(() => {
        fetch("" + empid).then((res) => {
            ...
            firstnamechange(resp.id_input_fm_1_firstname);
            lastnamechange(resp.id_input_fm_1_lastname);
            titlechange(resp.id_input_fm_1_title);
            genderchange(resp.id_input_fm_1_gender);
            nicchange(resp.id_input_fm_1_nic);
            dateofbirthchange(resp.id_input_fm_1_dateofbirth);
            contactnumberchange(resp.id_input_fm_1_contactnumber);
            emailchange(resp.id_input_fm_1_email);
            departmentchange(resp.id_input_fm_1_department);
            designationchange(resp.id_input_fm_1_designation);
            nationalitychange(resp.id_input_fm_1_nationality);
            addresschange(resp.id_input_fm_1_address);
            countrychange(resp.id_input_fm_1_country);
            citychange(resp.id_input_fm_1_city);
            profilepicturechange(resp.id_input_fm_1_profilepicture);
            descriptionchange(resp.id_input_fm_1_description);
            remarkschange(resp.id_input_fm_1_remarks);
            activechange(resp.id_input_fm_1_isactive);
        }).catch((err) => {
